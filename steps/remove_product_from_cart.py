...
# Classe com teste de remoção do produto no carrinho utilizando BDD
class RemoveProductFromCart:
    @given(u'que eu esteja com a home do portal americanas aberto @remove_product')
    ...
    @when(u'eu inserir o código do produto no campo de busca @remove_product')
    ...
    @when(u'clicar no botão de busca @remove_product')
    ...
    @then(u'a pesquisa é realizada e o produto corespondente é exibido na lista @remove_product')
    ...
    @then(u'selecionar o produto @remove_product')
    ...
    @then(u'a página com as informações para compra do produto é exibida @remove_product')
    def wait_product_page(context):
        Interact.delay(2)
        if not ProductPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'validar as informações do produto @remove_product')
    ...
    @then(u'clicar em comprar @remove_product')
    ...
    @then(u'a página de seleção da garantia e do seguro é exibida @remove_product')
    def wait_warranty_page(context):
        Interact.delay(3)
        if not WarrantyPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'clicar em continuar @remove_product')
    ...
    @then(u'a página do carrinho de compras é exibida com o produto adicionado @remove_product')
    def wait_cart_page(context):
        Interact.delay(3)
        if not CartPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'validar as informações do produto no carrinho de compras @remove_product')
    ...
    @then(u'clicar em remover @remove_product')
    def increment_product_quantity(self):
        Interact.click('xpath', CartPage.span_remove_product_element())

    @then(u'o produto é removido do carrinho de compras @remove_product')
    def wait_update_information(self):
        Interact.wait_visible_element('xpath', 3, CartPage.h2_empty_cart_element())

    @then(u'é exibida uma mensagem informando que nã há produtos no carrinho de compras @remove_product')
    def validate_new_product_info(self):
        ...
        # if not Interact.get_text('xpath', CartPage.h2_empty_cart_element()) == Interact.get_excel('validate', 3, 1):
        #    raise Exception('Produto não removido da cesta!')
        after_test()
