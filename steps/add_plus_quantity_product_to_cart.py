...
def before_test():
    Interact.setup(Browser.browser, Browser.headless)
...
# Classe com teste de adição do produto no carrinho incrementando quantidade utilizando BDD
class AddPlusQuantityProductToCart:
    @given(u'que eu esteja com a home do portal americanas aberto @add_product_plus')
    ...
    @when(u'eu inserir o código do produto no campo de busca @add_product_plus')
    ...
    @when(u'clicar no botão de busca @add_product_plus')
    ...
    @then(u'a pesquisa é realizada e o produto corespondente é exibido na lista @add_product_plus')
    ...
    @then(u'selecionar o produto @add_product_plus')
    ...
    @then(u'a página com as informações para compra do produto é exibida @add_product_plus')
    def wait_product_page(context):
        Interact.delay(2)
        if not ProductPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'validar as informações do produto @add_product_plus')
    ...
    @then(u'clicar em comprar @add_product_plus')
    ...
    @then(u'a página de seleção da garantia e do seguro é exibida @add_product_plus')
    def wait_warranty_page(context):
        Interact.delay(3)
        if not WarrantyPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'clicar em continuar @add_product_plus')
    ...
    @then(u'a página do carrinho de compras é exibida com o produto adicionado @add_product_plus')
    def wait_cart_page(context):
        Interact.delay(3)
        if not CartPage.title == Interact.get_title():
            raise Exception('Título da página não correspondente.')

    @then(u'validar as informações do produto no carrinho de compras @add_product_plus')
    ...
    @then(u'clicar no botão de + para incrementa a quantidade do produto no carrinho @add_product_plus')
    def increment_product_quantity(self):
        Interact.click('xpath', CartPage.svg_plus_quantity_element())

    @then(u'as informações são atualizadas @add_product_plus')
    def wait_update_information(self):
        Interact.wait_visible_element('xpath', 3, CartPage.div_cart_product_price_element())

    @then(u'validar as novas informações do produto no carrinho @add_product_plus')
    def validate_new_product_info(self):
        Interact.delay(2)
        # Por algum motivo o portal americanas não está deixando a função gettext retornar o texto do elemento
        # Porém as validações estão aqui nesta função para verificação
        ...
        # Validar preço do produto
        # if not Interact.get_text('xpath', CartPage.div_cart_product_price_element()) == Interact.get_excel('validate', 2, 1):
        #    raise Exception('Preço do produto não correspondente!')
        after_test()
